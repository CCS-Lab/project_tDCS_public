//m1
//codes from hBayesDM gng_m3.stan
data {
  int<lower=1> N;
  int<lower=1> T;
...
model {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:Tsubj[i]) {
      ...
      sv[cue_t] += ep[i] * (rho[i] * outcome_t - sv[cue_t]);

      // update action values
      if (pressed_t) { // update go value
        qv_g[cue_t] += ep[i] * (rho[i] * outcome_t - qv_g[cue_t]);
      } else { // update no-go value
        qv_ng[cue_t] += ep[i] * (rho[i] * outcome_t - qv_ng[cue_t]);
      }
    } // end of t loop
  } // end of i loop
}
