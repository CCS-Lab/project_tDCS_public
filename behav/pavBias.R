...
ggplot(data = data_subj_all, aes(y = pavBias_rew, x = db, fill = 'purple')) +
...
ggplot(data = data_subj_all, aes(y = pavBias_pun, x = db, fill = 'purple')) +
  ...
  geom_point(data = beeswarm_all, aes(y = y, x = x), color = "black", alpha = 0.5) +
  geom_line(aes(group = subject), alpha = 0.1) +
  stat_compare_means(comparisons = comp_all, paired = T, hide.ns = F, method = "t.test", label = "p.signif", label.y = 0.9) +
