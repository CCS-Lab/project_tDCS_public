...
## N = 27
anode_first = c(28, 18, 33, 60, 63, 64, 76, 91, 86, 59, 105, 107, 109, 112, 121, 126, 127) 
# go_reward < 0.1 : 61
...
#write.csv(data_subj_all,"data_subj_all_tmp2.csv", row.names = FALSE)
...
ggplot(data = data_subj_all, aes(y = pavBias, x = db, fill = 'purple')) +
  ...
  ggtitle(str_glue("Pavlovian bias (Sham N = {n_sham_first}, Anode N = {n_anode_first})")) +
...
ggplot(data = data_subj_all, aes(y = pavBias_rew, x = db, fill = 'pp')) +
  ...
  ggtitle(str_glue("Pavlovian bias (reward) (Sham N = {n_sham_first}, Anode N = {n_anode_first})")) +
  theme_classic2() + scale_fill_brewer(palette="Set3") 

### pavBias punish
beeswarm_all <- beeswarm(data = data_subj_all, pavBias_pun ~ db, method = 'swarm')
ggplot(data = data_subj_all, aes(y = pavBias_pun, x = db, fill = 'pp')) +
  ...
  ggtitle(str_glue("Pavlovian bias (punish) (Sham N = {n_sham_first}, Anode N = {n_anode_first})")) +
  theme_classic2() + scale_fill_brewer(palette="Set3") 
