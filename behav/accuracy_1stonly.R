...
# Check go_reward = 0 subjects (ses = 1) ###########
rawdata_subj <- data %>% group_by(subject, cond, db) %>% summarise(accuracy = mean(is_correct))
go_reward <- rawdata_subj %>% filter(cond == 'go_reward') %>% filter(accuracy < 0.1)
####################################################
...
ggplot(data = data_subj_all, aes(y = accuracy, x = db)) +
  ...
  stat_compare_means(comparisons = comp_all, paired = F, hide.ns = F, method = "t.test", label = "p.signif", label.y = 1.05) +
  ...
  ggtitle(str_glue("Overall accuracy (Sham N = {n_sham_first}, Anode N = {n_anode_first})"))
...
ggplot(data = data_subj_0, aes(y = accuracy, x = cond)) +
  ...
  theme_bw() + theme(legend.position = "none",
                     ...
                     axis.title.y = element_text(size = 13),
                     title = element_text(size = 15)) +
  ggtitle(str_glue("Sham: accuracy by condition (N = {n_sham_first})")) +
...
ggplot(data = data_subj_1, aes(y = accuracy, x = cond)) +
  ...
  theme_bw() + theme(legend.position = "none",
                     ...
                     axis.title.y = element_text(size = 13),
                     title = element_text(size = 15)) +
  ggtitle(str_glue("Anode: accuracy by condition (N = {n_anode_first})"))  +
  theme_classic2() + scale_fill_brewer(palette="Pastel1") 
