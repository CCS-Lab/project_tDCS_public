...
# Load data (N = 27)
load(str_glue('{SAVE_DIR}/fit_RData/fit_m4_diff_exp_2102_qc.RData'))
load(str_glue('{SAVE_DIR}/fit_RData/fit_m4_rew.pun_2102_qc_N25.RData'))
load(str_glue('{SAVE_DIR}/fit_RData/fit_c1.RData'))
...
## Save images
fits = c(fit_m5, fit_m6, fit_m7, fit_m8)
fits_n = c('fit_m5', 'fit_m6', 'fit_m7', 'fit_m8')
params = list(params_m5, params_m6, params_m7, params_m8)
...
for (i in 1:length(fits)){
  ...
  pdf(str_glue("{SAVE_DIR}/traceplots_{fits_n[i]}_N{N}_c1.pdf"))
...
for (i in 1:length(fits)){
  ...
  pdf(str_glue("{SAVE_DIR}/rhatplots_{fits_n[i]}_N{N}_c1.pdf"))
...
loo_compare(loo(extract_log_lik(fit_m4_nod_qc)),
            ...
            loo(extract_log_lik(fit_m4_Pav_qc)),
            loo(extract_log_lik(fit_m4_GoPav_qc)),
            loo(extract_log_lik(fit_m5)),
            loo(extract_log_lik(fit_m6)),
            loo(extract_log_lik(fit_m7)),
            loo(extract_log_lik(fit_m8)),
...
df3 <- data.frame(b_0=colMeans(ext_m4_GoPav_qc$b_0),
                      b_delta=colMeans(ext_m4_GoPav_qc$b_delta),
                      pi_0=colMeans(ext_m4_GoPav_qc$pi_0),
