...
# Load data (N = 27)
load(str_glue('{SAVE_DIR}/fit_RData/fit_prior-fixed.RData'))
...
fits = c(fit_m4_GoPav_qc)
fits_n = c('fit_m4_GoPav_qc')
params = list(params_GoPav)
...
for (i in 1:length(fits)){
  ...
  pdf(str_glue("{SAVE_DIR}/rhatplots_{fits_n[i]}_N{N}.pdf"))
...
mcmc_areas(fit_m4_GoPav_qc, regex_pars = "xi\\[.*", prob = 0.95, prob_outer = 1) + xlim(0,0.4)
mcmc_areas(fit_m4_GoPav_qc, regex_pars = "xi$", prob = 0.95, prob_outer = 1) + xlim(0,0.4)
mcmc_areas(fit_m4_GoPav_qc, regex_pars = "ep\\[.*", prob = 0.95, prob_outer = 1) + xlim(0,1)
mcmc_areas(fit_m4_GoPav_qc, regex_pars = "ep$", prob = 0.95, prob_outer = 1) + xlim(0,1)
mcmc_areas(fit_m4_GoPav_qc, regex_pars = "rhoRew\\[.*", prob = 0.95, prob_outer = 1) + xlim(0,50)
mcmc_areas(fit_m4_GoPav_qc, regex_pars = "rhoRew$", prob = 0.95, prob_outer = 1) + xlim(0,50)
mcmc_areas(fit_m4_GoPav_qc, regex_pars = "rhoPun\\[.*", prob = 0.95, prob_outer = 1) + xlim(0,50)
mcmc_areas(fit_m4_GoPav_qc, regex_pars = "rhoPun$", prob = 0.95, prob_outer = 1) + xlim(0,50)
dev.off()
