...
parameters {
  ...
  vector[N] k_0_rew_pr;      // base pavlovian bias
  vector[N] k_delta_rew_pr;  // change pavlovian bias
  vector[N] k_0_pun_pr;      // base pavlovian bias
  vector[N] k_delta_pun_pr;  // change pavlovian bias
}
transformed parameters {
  ...
  vector[N] k_0_rew;
  vector[N] k_delta_rew;
  vector[N] k_0_pun;
  vector[N] k_delta_pun;
  ...
  k_0_rew  = mu_p[11] + sigma[11] * k_0_rew_pr;
  k_delta_rew  = mu_p[12] + sigma[12] * k_delta_rew_pr;
  k_0_pun  = mu_p[13] + sigma[13] * k_0_pun_pr;
  k_delta_pun  = mu_p[14] + sigma[14] * k_delta_pun_pr;
...
// gng_m4: RW(rew/pun) + noise + bias + pi model (M5 in Cavanagh et al 2013 J Neuro)
  ...
  k_0_rew_pr  ~ normal(0, 1.0); 
  k_delta_rew_pr  ~ normal(0, 1.0); 
  k_0_pun_pr  ~ normal(0, 1.0); 
  k_delta_pun_pr  ~ normal(0, 1.0); 
  ...
  for (i in 1:N) {
    ...
    for (blk in 1:B) {
      ...
      if (blk==1) {
        ...
        inst_bias_rew = k_0_rew[i];
        inst_bias_pun = k_0_pun[i];
      } else if (blk==2) {
        go_bias = b_0[i] + b_delta[i];
        pav_bias_rew = pi_0_rew[i] + pi_delta_rew[i];
        pav_bias_pun = pi_0_pun[i] + pi_delta_pun[i];
        inst_bias_rew = k_0_rew[i] + k_delta_rew[i];
        inst_bias_pun = k_0_pun[i] + k_delta_pun[i];
      }
...
generated quantities {
  ...
  mu_k_0_rew  = mu_p[11];
  mu_k_delta_rew  = mu_p[12];
  mu_k_0_pun  = mu_p[13];
  mu_k_delta_pun  = mu_p[14];
