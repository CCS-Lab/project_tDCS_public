...
parameters {
  ...
  vector[N] pi_1_pr;      // base pavlovian bias
  vector[N] pi_2_pr;  // change pavlovian bias 
...
transformed parameters {
  ...
  pi_1  = mu_p[5] + sigma[5] * pi_1_pr;
  pi_2  = mu_p[6] + sigma[6] * pi_2_pr;
...
// gng_m4: RW(rew/pun) + noise + bias + pi model (M5 in Cavanagh et al 2013 J Neuro)
  ...
  b_1_pr    ~ normal(0, 1.0); 
  b_2_pr    ~ normal(0, 1.0); 
  pi_1_pr   ~ normal(0, 1.0); 
  pi_2_pr   ~ normal(0, 1.0); 
...
generated quantities {
  real<lower=0, upper=1> mu_xi = Phi_approx(mu_p[1]);
  real<lower=0, upper=1> mu_ep = Phi_approx(mu_p[2]);
  real mu_b_1 = mu_p[3];
  real mu_b_2 = mu_p[4];
  real mu_pi_1 = mu_p[5];
  real mu_pi_2 = mu_p[6];
  real<lower=0> mu_rhoRew = exp(mu_p[7]);
  real<lower=0> mu_rhoPun = exp(mu_p[8]); 
}
