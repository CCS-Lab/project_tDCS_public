...
transformed parameters {
  ...
  kRew   = Phi_approx(mu_pr[6] +sigma[6] * kRew_pr) - 0.5;
  kPun   = Phi_approx(mu_pr[7] +sigma[7] * kPun_pr) - 0.5;
  
  {
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        wv_g[cue_t]  = qv_g[cue_t] + b[i] + sv[cue_t];
...
generated quantities {
  ...
  mu_kRew   = Phi_approx(mu_pr[6]) - 0.5;
  mu_kPun   = Phi_approx(mu_pr[7]) - 0.5;
}
