...
transformed parameters {
  ...
  kRew   = Phi_approx(mu_pr[7] +sigma[7] * kRew_pr) - 0.5;
  kPun   = Phi_approx(mu_pr[8] +sigma[8] * kPun_pr) - 0.5;
...
// gng_m4: RW(rew/pun) + noise + bias + pi model (M5 in Cavanagh et al 2013 J Neuro)
  ...
  sigma[4:6] ~ normal(0, 0.2);
  sigma[7:8] ~ normal(0, 0.5); // SD 3
...
generated quantities {
  ...
  mu_kRew   = Phi_approx(mu_pr[7]) - 0.5;
  mu_kPun   = Phi_approx(mu_pr[8]) - 0.5;
}
