...
parameters {
  ...
  vector[N] epPun_pr;         // learning rate
  vector[N] b_pr;          // go bias
  vector[N] pi_pr;         // pavlovian bias
  vector[N] rho_pr;     // rho reward, inv temp
}
...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    epPun[i]  = Phi_approx(mu_pr[4] + sigma[4] * epPun_pr[i]);
  }
  b      = mu_pr[5] + sigma[5] * b_pr; // vectorization
  pi     = mu_pr[6] + sigma[6] * pi_pr;
  rho = exp(mu_pr[7] + sigma[7] * rho_pr);
  
  {
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        sv[cue_t] += lr * (rho[i] * outcome_t - sv[cue_t]);
  
        // update action values
        if (pressed_t) { // update go value
          qv_g[cue_t] += lr * (rho[i] * outcome_t - qv_g[cue_t]);
        } else { // update no-go value
          qv_ng[cue_t] += lr * (rho[i] * outcome_t - qv_ng[cue_t]);
        }
      } // end of t loop
    } // end of i loop
  }
