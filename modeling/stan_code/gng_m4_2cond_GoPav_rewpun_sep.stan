...
parameters {
  ...
  vector[N] b_1_pr;       // baseline go bias 
  vector[N] b_2_pr;   // change go bias 
  vector[N] piRew_1_pr;      // base pavlovian bias
  vector[N] piRew_2_pr;  // change pavlovian bias
  vector[N] piPun_1_pr;      // base pavlovian bias
  vector[N] piPun_2_pr;  // change pavlovian bias 
...
transformed parameters {
  ...
  b_1       = mu_p[3] + sigma[3] * b_1_pr; // vectorization
  b_2       = mu_p[4] + sigma[4] * b_2_pr;
  piRew_1   = mu_p[5] + sigma[5] * piRew_1_pr;
  piRew_2   = mu_p[6] + sigma[6] * piRew_2_pr;
  piPun_1   = mu_p[7] + sigma[7] * piPun_1_pr;
  piPun_2   = mu_p[8] + sigma[8] * piPun_2_pr;
  ...
  
  {
    for (i in 1:N) {
      ...
      for (blk in 1:B) {
        ...
        if (blk==1) {
          go_bias = b_1[i];
          pav_bias_rew = piRew_1[i];
          pav_bias_pun = piPun_1[i];
        } else if (blk==2) {
          go_bias = b_2[i];
          pav_bias_rew = piRew_2[i];;
          pav_bias_pun = piPun_2[i];
        }
...
// gng_m4: RW(rew/pun) + noise + bias + pi model (M5 in Cavanagh et al 2013 J Neuro)
  ...
  mu_p[3]  ~ normal(0, 10.0); // b_1
  mu_p[4]  ~ normal(0, 10.0); // b_2
  mu_p[5]  ~ normal(0, 10.0); // piRew_1
  mu_p[6]  ~ normal(0, 10.0); // piRew_2
  mu_p[7]  ~ normal(0, 10.0); // piPun_1
  mu_p[8]  ~ normal(0, 10.0); // piPun_2
  mu_p[9]  ~ normal(0, 1.0); // rhoRew
  mu_p[10] ~ normal(0, 1.0); // rhoPun
  sigma[1:2]  ~ normal(0, 0.2);
  sigma[3:8]  ~ cauchy(0, 1.0);
  sigma[9:10] ~ normal(0, 0.2);
  ...
  b_1_pr       ~ normal(0, 1.0); 
  b_2_pr       ~ normal(0, 1.0); 
  piRew_1_pr   ~ normal(0, 1.0); 
  piRew_2_pr   ~ normal(0, 1.0); 
  piPun_1_pr   ~ normal(0, 1.0); 
  piPun_2_pr   ~ normal(0, 1.0); 
