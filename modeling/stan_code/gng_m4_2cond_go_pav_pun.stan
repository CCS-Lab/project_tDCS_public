...
transformed parameters {
  ...
  pi_rew        = mu_p[5] + sigma[5] * pi_rew_pr;
  pi_0_pun      = mu_p[6] + sigma[6] * pi_0_pun_pr;
  pi_delta_pun  = mu_p[7] + sigma[7] * pi_delta_pun_pr;
  rhoRew        = exp( mu_p[8] + sigma[8] * rhoRew_pr );
  rhoPun        = exp( mu_p[9] + sigma[9] * rhoPun_pr );
...
// gng_m4: RW(rew/pun) + noise + bias + pi model (M5 in Cavanagh et al 2013 J Neuro)
  ...
  mu_p[3]  ~ normal(0, 10.0); // b_0
  mu_p[4]  ~ normal(0, 1.0);  // b_delta
  mu_p[5]  ~ normal(0, 10.0); // pi_rew
  mu_p[6]  ~ normal(0, 10.0); // pi_pun_0
  mu_p[7]  ~ normal(0, 1.0); // pi_pun_delta
  mu_p[8]  ~ normal(0, 1.0); // rhoRew
  mu_p[9] ~ normal(0, 1.0); // rhoPun
  sigma[1:2]  ~ normal(0, 0.2);
  sigma[3]    ~ cauchy(0, 1.0);
  sigma[4]    ~ normal(0, 0.2);
  sigma[5:6]  ~ cauchy(0, 1.0);
  sigma[7:9] ~ normal(0, 0.2);
...
generated quantities {
  ...
  mu_pi_rew        = mu_p[5];
  mu_pi_0_pun      = mu_p[6];
  mu_pi_delta_pun  = mu_p[7];
  mu_rhoRew        = exp(mu_p[8]);
  mu_rhoPun        = exp(mu_p[9]); 
}
