  ...
  
  {
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        pressed_t ~ bernoulli( pGo[ cue_t ] );
        
        // after receiving feedback, update sv[t + 1]
        if (outcome_t >= 0) {
          sv[cue_t] += ep[i] * (rhoRew[i] * outcome_t - sv[cue_t]);
        } else {
          sv[cue_t] += ep[i] * (rhoPun[i] * outcome_t - sv[cue_t]);
        }
  
        // update action values
        if (pressed_t) { // update go value
          if (outcome_t >=0) {
            qv_g[cue_t] += ep[i] * (rhoRew[i] * outcome_t - qv_g[cue_t]);
          } else {
            qv_g[cue_t] += ep[i] * (rhoPun[i] * outcome_t - qv_g[cue_t]);
          }
        } else { // update no-go value
          if (outcome_t >=0) {
            qv_ng[cue_t] += ep[i] * (rhoRew[i] * outcome_t - qv_ng[cue_t]);
          } else {
            qv_ng[cue_t] += ep[i] * (rhoPun[i] * outcome_t - qv_ng[cue_t]);
          }
        }
      } // end of t loop
    } // end of i loop
  ...
  { // local section, this saves time and space
    for (i in 1:N) {
      ...
      for (t in 1:Tsubj[i]) {
        ...
        Qgo[i, t]   = qv_g[cue_t];
        Qnogo[i, t] = qv_ng[cue_t];
        Wgo[i, t]   = wv_g[cue_t];
        Wnogo[i, t] = wv_ng[cue_t];
        SV[i, t]    = sv[cue_t];
