...
load(str_glue('{SAVE_DIR}/fit_m4_sep_exp_2102_qc.RData'))
load(str_glue('{SAVE_DIR}/fit_RData/fit_m4_GoPav_rewpun_sep.RData'))
...
## Save images
fits = c(fit_m4_nod_qc, fit_m4_Go_qc, fit_m4_Pav_qc, fit_m4_GoPav_qc, fit_m4_Go_sep_qc, fit_m4_Pav_sep_qc, fit_m4_GoPav_sep_qc)
fits_n = c('fit_m4_nod_qc', 'fit_m4_Go_qc', 'fit_m4_Pav_qc', 'fit_m4_GoPav_qc', 'fit_m4_Go_sep_qc', 'fit_m4_Pav_sep_qc', 'fit_m4_GoPav_sep_qc')
params = list(params_nod, params_Go, params_Pav, params_GoPav, params_Go_sep, params_Pav_sep, params_GoPav_sep)
...
for (i in 1:length(fits)){
  ...
  pdf(str_glue("{SAVE_DIR}/traceplots_{fits_n[i]}_N{N}_prior-fixed.pdf"))
...
for (i in 1:length(fits)){
  ...
  pdf(str_glue("{SAVE_DIR}/rhatplots_{fits_n[i]}_N{N}_prior-fixed.pdf"))
...
# 
## b
ext <- rstan::extract(it_m4_GoPav_sep_qc)
ind_b_del <- as.data.frame(ext$b_2 - ext$b_1)
...
mu_b_del <- as.data.frame(ext$mu_b_2 - ext$mu_b_1); names(mu_b_del) <- 'mu_b'
b_del <- cbind(mu_b_del, ind_b_del)

ind_pi_del <- as.data.frame(ext$pi_2 - ext$pi_1)
...
mu_pi_del <- as.data.frame(ext$mu_pi_2 - ext$mu_pi_1); names(mu_pi_del) <- 'mu_pi'
pi_del <- cbind(mu_pi_del, ind_pi_del)

mcmc_areas(mu_b_del, prob = 0.95, prob_outer = 1)
mcmc_areas(mu_pi_del, prob = 0.95, prob_outer = 1)

## piRew
ind_piRew_del <- as.data.frame(ext$piRew_2 - ext$piRew_1)
...
mu_piRew_del <- as.data.frame(ext$mu_piRew_2 - ext$mu_piRew_1); names(mu_piRew_del) <- 'mu_piRew'
piRew_del <- cbind(mu_piRew_del, ind_piRew_del)

mcmc_areas(mu_piRew_del, prob = 0.95, prob_outer = 1)

## piPun
ind_piPun_del <- as.data.frame(ext$piPun_2 - ext$piPun_1)
...
mu_piPun_del <- as.data.frame(ext$mu_piPun_2 - ext$mu_piPun_1); names(mu_piPun_del) <- 'mu_piPun'
