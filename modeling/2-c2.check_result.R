...
load(str_glue('{SAVE_DIR}/fit_RData/fit_m4_rew.pun_2102_qc.RData'))
load(str_glue('{SAVE_DIR}/fit_RData/fit_c1.RData'))
load(str_glue('{SAVE_DIR}/fit_RData/fit_c2.RData'))
...
## Save images
fits = c(fit_m14, fit_m16, fit_m20)
fits_n = c('fit_m14', 'fit_m16', 'fit_m20')
params = list(params_m14, params_m16, params_m20)

fits = c(fit_m14)
fits_n = c('fit_m14')
params = list(params_m14)
...
for (i in 1:length(fits)){
  ...
  pdf(str_glue("{SAVE_DIR}/estimate_plots/traceplots_{fits_n[i]}_N{N}_prior-fixed.pdf"))
...
for (i in 1:length(fits)){
  ...
  pdf(str_glue("{SAVE_DIR}/rhatplots_{fits_n[i]}_N{N}_c2.pdf"))
...
loo_compare(loo(extract_log_lik(fit_m4_GoPav_qc)),
            loo(extract_log_lik(fit_m20)))
